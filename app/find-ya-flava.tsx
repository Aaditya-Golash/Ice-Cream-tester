...
function SpoonVisual({ preferences, compact = false }: { preferences: Preferences; compact?: boolean }) {
  ...
  return (
    <div className={`spoon-visual ${compact ? 'compact-spoon' : ''}`}>
      ...
      <p><span aria-hidden="true" />Your request, illustrated—not a guarantee of what’s inside a pint.</p>
    </div>
  );
}
    ...
    <>
      <section className="hero" aria-labelledby="hero-title">
        <div className="hero-copy">
          ...
          <h1 id="hero-title">What’s in your<br /><em>perfect spoonful?</em></h1>
          <p className="hero-deck">Pick the flavour, the pieces, and how you like it melted. We’ll find the closest pint in this Dr. Bombay selection.</p>
      ...
      <section className="signature-idea" aria-labelledby="signature-title">
        ...
        <div>
          <p className="question-number">WHEN THE FREEZER DOESN’T HAVE IT YET</p>
        ...
        <section className="quiz-panel" aria-live="polite">
            ...
            <>
              ...
              {preferences.flavours.includes('chocolate') && (
                <div className="follow-up">
                  <h3>Where do you want it? <span>Optional</span></h3>
              ...
              <div className="follow-up">
                <h3>What should we leave out? <span>Optional</span></h3>
          ...
          {step === 2 && (
            <>
              ...
              {preferences.ribbon !== 'none' && <div className="follow-up"><h3>How much?</h3><div className="segmented">{([['little','A little'],['thick','A thick ribbon']] as const).map(([key,label]) => <button key={key} className={preferences.ribbonAmount === key ? 'active' : ''} onClick={() => set('ribbonAmount', key)}>{label}</button>)}</div></div>}
              <div className="follow-up">
                <h3>When do you start eating?</h3>
                <p className="microcopy">We’ll remember how you like it served.</p>
              ...
              <details className="details-box">
                <summary>The ice cream itself? <span>Optional detail</span></summary>
                <p>We’ll record this preference, but the catalogue does not verify product density.</p>
          ...
          {step === 3 && (
            <>
              ...
              <div className="follow-up">
                <h3>How should the next option branch out?</h3>
...
function ResultView({ preferences, result, onEdit, onExplore }: { preferences: Preferences; result: MatchResult | null; onEdit: () => void; onExplore: () => void }) {
  ...
  const share = async () => {
    ...
    try {
      if (navigator.share) {
        ...
        setShareStatus('Share link copied—city, free text, and feedback were left out.');
      }
    } catch {
  ...
  const downloadCard = async () => {
    try {
      ...
      setShareStatus('Your 1080 × 1350 “My Flava” card is ready to post.');
    } catch {
      setShareStatus('The card could not be created in this browser. Your private-safe result link still works.');
    }
  };
  ...
  if (!shown) {
    return (
      <main className="result-page empty-result">
        ...
        <p>We won’t recommend a product that conflicts with a documented ingredient preference. Save the combination as a request or adjust an answer.</p>
    ...
    <main className="result-page">
      ...
      <section className="evidence-grid">
        ...
        <article className="evidence-card unmet">
          <p className="card-label">WHAT IT DOESN’T COVER</p>
